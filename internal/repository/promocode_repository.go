...
// promocodeRepository реализует domain.PromocodeRepository
type promocodeRepository struct {
	dbGetter DBGetter
}

// NewPromocodeRepository создает новый репозиторий промокодов
func NewPromocodeRepository(dbGetter DBGetter) domain.PromocodeRepository {
...
func (r *promocodeRepository) Create(ctx context.Context, promocode *domain.Promocode) error {
	query := `
...
func (r *promocodeRepository) Update(ctx context.Context, promocode *domain.Promocode) error {
	query := `
		UPDATE promocodes
...
func (r *promocodeRepository) GetActive(ctx context.Context) ([]*domain.Promocode, error) {
	query := `
		SELECT id, code, type, value, is_active, usage_limit, used_count, expires_at, created_at, updated_at
		FROM promocodes
...
func (r *promocodeRepository) scanPromocode(row pgx.Row) (*domain.Promocode, error) {
